...
N_list_merge = np.linspace(10, 10000, 20, dtype=int)
bubble_times = []
merge_times = []
...
for N in N_list_bubble:
    ...
    for i in range(5):
        ...
        total_time_bubble += timeit.timeit(
            "bubble_sort(number_list)", globals=globals(), number=1
        )
...
for N in N_list_merge:
    ...
    for i in range(5):
        ...
        b_list = rand.randint(int(-2), int(2), size=N)
        number_list = [HPNumber(a_list[i], b_list[i]) for i in range(N)]
        total_time_merge += timeit.timeit(
            "merge_sort(number_list)", globals=globals(), number=1
        )
...
plt.savefig("Bubble-Proportional.png")
plt.show()
...
plt.plot(
    N_list_merge,
    np.array(merge_times) / (np.array(N_list_merge) * np.log(np.array(N_list_merge))),
)
plt.savefig("Merge-Proportional.png")
plt.show()
